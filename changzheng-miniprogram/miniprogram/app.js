...
App({
          ...
          if (res.code) {
            try {
              ...
              if (loginRes.code === 200) {
                const { token, userInfo, openId } = loginRes.data
                
                // 合并用户信息：保留本地已设置的头像和昵称
                const mergedUserInfo = {
                  ...userInfo,
                  // 如果本地有头像昵称，优先使用本地的
                  avatarUrl: storedUserInfo.avatarUrl || userInfo.avatarUrl,
                  nickName: storedUserInfo.nickName || userInfo.nickName
                }
                ...
                resolve({ ...loginRes.data, userInfo: mergedUserInfo })
              } else {
  ...
  // 登出（只清除token，保留用户设置的头像、昵称、学生认证信息）
  logout() {
    this.globalData.token = null
    this.globalData.isLogin = false
    // 只清除token，保留userInfo和openId
    wx.removeStorageSync('token')
    // 不清除userInfo和openId，这样重新登录时可以读取之前的设置
  },
